...
tm_dots() # defines howto plot the second object
```
...
tm_shape(world_vector) +
  tm_polygons(fill = "life_expectancy",
              ...
              fill.legend = tm_legend(title = "Life Expentancy (years)", 
                                      position = tm_pos("left")))
```
...
tm_shape(world_vector) +
  tm_fill() +
  tm_scalebar() +
  tm_compass(north = 180, position = c("left", "top"))
```
...
tmap_save(map1, here::here("week1", "~/Documents/MEDS/EDS-223/EDS223-in-class-labs/week1"))
```
